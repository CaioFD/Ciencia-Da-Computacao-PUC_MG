...
void imprimir2 ( int id, Personagem *perso )//imprimir um personagem dnv
{
...
void deleteNoRec ( No* no )
{
    if( no != NULL )
    {
        deleteNoRec(no->esq);
        deleteNoRec(no->dir);
        delete_Personagem(no->elemento);
        free( no ); no = NULL;
    } 
} // end deleteNoRec ( )
...
{
    deleteNoRec( avl->raiz );
    free( avl ); avl = NULL;
} // end delete_AVL ( )

bool pesquisarRec ( No* i, char* nome, Log* log )
{
    bool resp = false;
    if ( i == NULL ) {
        log->comparacoes++;
...
No* balancear ( No* no )
{
    if( no != NULL )
    {
        ...
        if ( abs(fator) <= 1 ) {
            ...
            int fatorFilhoEsq = get_fator_No( no->esq );
            if( fatorFilhoEsq == 1 ) {
                no->esq = rotacionarEsq( no->esq );
            } 
            no = rotacionarDir( no );
        } else {
            printf( "Erro no No( %s ) com fator de balanceamento ( %d ) invalido!", getName( no->elemento ), fator );
        } 
    } 
    return ( no );
} // end balancear ( )

No* inserirRec( No* i, Personagem* perso )
{
    if ( i == NULL ) {
        i = new_No( perso );
    } else if ( strcmp(getName(perso), getName(i->elemento)) < 0 ) {
        i->esq = inserirRec( i->esq, perso );
    } else if ( strcmp(getName(perso), getName(i->elemento)) > 0 ) {
        i->dir = inserirRec( i->dir, perso );
    } else {
        printf( "\n%s\n", "ERRO ao inserir!" );
    } 
    return ( balancear(i) );
...
void inserir ( AVL* avl, Personagem* perso )
{
    avl->raiz = inserirRec( avl->raiz, perso );
} // end inserir ( )
...
int main ( void )
{
    ...
    {
        bool resp = pesquisar( avl, nome, &log );
        if( resp == true ) {
            printf( "%s\n", "SIM" );
        }
        else {
    ...
    end_Timer( &timer );
    registro( "79360_avl.txt", &timer, &log );
