...
Personagem pesquisaporId(char *id)
{
    ...
    char aux[1000];
    FILE *arquivo = fopen("/tmp/characters.csv", "r");
    fgets(aux, sizeof(aux), arquivo); // ignora o cabeçalho
    ...
    int k = 0, j = 0, cont = 0; //preenchimento dos campos
    for (int i = 0; i < strlen(personagem2); i++)
    {
        if (personagem2[i] != ';')
        { // Verifica se não é o delimitador
...
int pesquisar(Personagem personagem[], int indice)//pesquisa
{
    char id[100];
...
void insertionWithColor(int cor, int h, Personagem personagem[], int indice)// INSERE COM BASE NA COR
{
...
void start()
{
    Personagem personagem;
    for(int i = 0; i<21; i++){
        tabela[i].primeiro = tabela[i].ultimo = novaCelula(personagem);
    }
}
...
void inserirHash(Personagem personagem)
{
    int indice = funcaoHash(personagem.name);
    tabela[indice].ultimo->prox = novaCelula(personagem);
    tabela[indice].ultimo = tabela[indice].ultimo->prox;
}

bool pesquisarHash(char nome[]) {
    int indice = funcaoHash(nome);
    for (Celula *i = tabela[indice].primeiro->prox; i != NULL; i = i->prox) {
        if (strcmp(i->personagem.name, nome) == 0) {
            return true;
        }
    }
    return false;
...
int main()
{
    ...
    {
        scanf(" %[^\r\n]", id);
        if (strcmp(id, "FIM") == 0)//is fim
        {
            while(1){
                scanf(" %[^\r\n]", name[j].name);
                if(strcmp(name[j].name, "FIM") == 0){
                    break;
                }
                j++;
            }
    ...
    for(int k = 0; k<i; k++){
        inserirHash(personagem[k]);
    }

    for(int k = 0; k<j; k++){
        bool resp = pesquisarHash(name[k].name);
        if(resp){
            int hash = funcaoHash(name[k].name);
            printf("%s (pos: %d) SIM\n", name[k].name, hash);
        }else{
            printf("%s NAO\n", name[k].name);
        }
