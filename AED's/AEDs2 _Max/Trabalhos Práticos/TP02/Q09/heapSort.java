...
setWizard          ( wizard );
   ...
   //--------- Métodos Uteis ---------//
    ...
    public static int CompararCabelo( Personagem perso1, Personagem perso2 )
    {
        int result = 0;
        String hair1 = perso1.getHairColour();
        String hair2 = perso2.getHairColour();

        if (hair1.compareTo(hair2) < 0) {
            result = -1;
        }
        else if (hair1.compareTo(hair2) > 0){
            result = 1;
        }
        else{
            if( perso1.getName().compareTo(perso2.getName()) > 0 ){
        ...
        return ( result );
                ...
                public static void heapSort(List<Personagem> perso, Log log) {
                    ...
                    for( int i = n / 2 - 1; i >= 0; i = i -1 ) { // construo 
                        reconstruir( perso, i, n, log );
                    }
            
                    for( int i = n - 1; i > 0; i = i -1 ) { // ordenaçao propriamente dita
                        swap(0, i, perso, log);
                ...
                private static void reconstruir(List<Personagem> perso, int i, int n, Log log) {
                    ...
                    if( esquerda < n && CompararCabelo(perso.get(esquerda), perso.get(filho)) > 0 ) 
                    {
                        filho = esquerda;
                        log.incrementarComp( );
                    } // end if
            
                    if( direita < n && CompararCabelo(perso.get(direita), perso.get(filho)) > 0 ) 
                    {
                        filho = direita;
                        log.incrementarComp( );
                    } // end if
...
public class heapSort extends Personagem
{
    public static void main ( String [] args )
    {
        Scanner scan = new Scanner( System.in );
        
        Log log = new Log( "793605_heapsort.txt" );
        Personagem perso = new Personagem( );
