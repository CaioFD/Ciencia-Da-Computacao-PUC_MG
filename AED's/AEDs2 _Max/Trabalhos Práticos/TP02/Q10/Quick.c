 ...
 *  QuickSort.
*/
void quicksortRec( Personagem** perso, int esq, int dir, Log* log ) 
{
    int i = esq, j = dir;
    ...
    if( esq < j ) {
        log->comparacoes++;
        quicksortRec( perso, esq, j, log );
    } // end if
    if( i < dir ) {
        log->comparacoes++;
        quicksortRec( perso, i, dir, log );
    } // end if
} // end quicksortRec ( )
...
*/
void quicksort( Personagem** perso, int length, Log * log ) 
{
    quicksortRec( perso, 0, length-1, log );
...
int main ( void )
{
    ...
    start_Timer( &timer );
    quicksort( perso, tam, &log );
    end_Timer( &timer );
    registro( "793605_quicksort.txt", &timer, &log );
