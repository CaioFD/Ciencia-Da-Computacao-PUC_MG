...
void selecaoRec( int tam, Personagem** perso, Log *log, int i, int j )
{
    if( i < tam )
    {
        if( j < tam )
        {
            if(  strcmp( getName( perso[j] ), getName( perso[i] ) ) < 0 )
            {
                log->comparacoes++;
                swap( i, j, perso );
                log->movimentacoes += 3;
            ...
            log->comparacoes++;
            selecaoRec( tam, perso, log, i, j+1 );
        }
        else
        {
            selecaoRec( tam, perso, log, i+1, i+2 );
            log->comparacoes++;
        } // end if
    } // end if
} // end selecaoRec ( )
...
void selecao( int tam, Personagem** perso, Log *log )
{
    selecaoRec( tam, perso, log, 0, 1 );
} // end selecao ( )
...
int main ( void )
{
    ...
    start_Timer( &timer );
    selecao( tam, perso, &log );
    end_Timer( &timer );
    registro( "793605_selecaoRecursiva.txt", &timer, &log );
