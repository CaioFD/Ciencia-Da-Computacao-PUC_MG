...
//Iniciliza personagem com um valor
                ...
                if( index < 18 )
                {
                    if( index > 16 ) 
                    {
                        ...
                        tratamentoData(atributos[11]),
                                ...
                                strcmp(atributos[16], "VERDADEIRO") == 0 );
                    } 
                    else
                    {
                        init_Personagem(perso,
                        ...
                        tratamentoData(atributos[10]),
                                ...
                                strcmp(atributos[15], "VERDADEIRO") == 0 );
                    } 
                }
                else
                {
                    ...
                    tratamentoData(atributos[12]),
                            atoi(atributos[13]), 
                                    ...
                                    atributos[16], 
                            strcmp(atributos[17], "VERDADEIRO") == 0 );
                } 
            } 
...
void ordenarPersonagem ( int tam, Personagem** perso, Log *log )
{
    for( int i = 0; i < tam; i = i + 1 )
    {
        ...
        {
            if( strcmp( getName( perso[j] ), getName( perso[menor] ) ) < 0 )
            {
                menor = j;
                log->comparacoes++;
            }
...
int pesquisaBinariaRec ( char* nome, int esq, int dir, Personagem** perso, Log *log )
{
    if( esq > dir ) {
        return ( -1 ); // not found
        log->comparacoes++;
    ...
    {
        int meio = ( esq + dir ) / 2;
        int comp = strcmp( nome, getName( perso[meio] ) );
        if( comp == 0 ) {
            return ( meio );    
            log->comparacoes++;
        } else if( comp > 0 ) {
            return pesquisaBinariaRec( nome, meio+1, dir, perso, log ) ;    
            log->comparacoes++;
        } else {
            return pesquisaBinariaRec( nome, esq, meio-1, perso, log );
        } 
    } 
} 

int pesquisaBinaria ( char* nome, int esq, int dir, Personagem **perso, Log *log )
{
    ordenarPersonagem( dir, perso, log );
    return ( pesquisaBinariaRec( nome, esq, dir-1, perso, log ) );
} 
...
int main ( void )
{
    ...
    registro( "812839_binaria.txt", &timer, &log );
