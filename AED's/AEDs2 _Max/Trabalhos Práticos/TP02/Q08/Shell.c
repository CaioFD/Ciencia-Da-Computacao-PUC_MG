...
int compareEyeColour( Personagem* perso1, Personagem* perso2 )
{
    int result = 0;
    char* eye1 = getEyeColour( perso1 );
    char* eye2 = getEyeColour( perso2 );
    if( strcmp( eye1, eye2 ) < 0 ) {
        result = -1;
    }
    else if( strcmp( eye1, eye2 ) > 0 ) {
        result = 1;
    }
    else 
    {
    ...
    return ( result );
} // end compareEyeColour ( )

void insercao( Personagem** perso, int length, int startIndex, int gap, Log* log )
{
    for( int i = ( gap + startIndex ); i < length; i = i + gap ) 
    {
        Personagem* tmp = perso[i];
        int j = i - gap;
        while( ( j >= 0 ) && compareEyeColour( perso[j], tmp ) > 0 ) 
        {
            perso[j+gap] = perso[j];
            j = j - gap;
            log->movimentacoes++;
...
void shellsort( Personagem** perso, int length, Log* log ) 
{
    int gap = 1;

    do { gap = ( gap * 3 ) + 1; } while( gap < length );

    do {
        gap = gap / 3;
        for( int startIndex = 0; startIndex < gap; startIndex++ ) {
            insercao( perso, length, startIndex, gap, log );
        } // end for
    } while( gap != 1 );
} // end shellsort ( )
...
int main ( void )
{
    ...
    start_Timer( &timer );
    shellsort( perso, tam, &log );
    end_Timer( &timer );
    registro( "793605_shell.txt", &timer, &log );
