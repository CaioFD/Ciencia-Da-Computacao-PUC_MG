...
typedef struct{//Strcut da data de nascimento
    int dia, mes, ano;
} LocalDate;



typedef struct{//Struct do Personagem
...
LocalDate convertData(char data[])//retorna um LocalDate de acordo com o parametro char[]
{
    LocalDate localDate;
...
void setPersonagem(Personagem *p, char *line){//seta um personagem de acordo com a linha char*
...
void arrayOfPersonagens(Personagem p[], int key){//Guarda todos os Personagens do csv em um array
    char path[100];
    if (key == 1){
...
//Imprime na tela o Personagem do parâmetro
